...
let isGameActive = true; // Variabile di stato per controllare se il gioco è attivo

function calculateBombsNearby(index, numSquare, squareWidth, bombs) {
  const row = Math.floor(index / squareWidth);
  const col = index % squareWidth;
  let count = 0;
  ...
  for (let i = -1; i <= 1; i++) {
    for (let j = -1; j <= 1; j++) {
      ...
      if (
        ...
        const newIndex = newRow * squareWidth + newCol;
        if (bombs.includes(newIndex + 1)) count++;
      }
    }
  }
...
btn.addEventListener("click", function () {
  resetGame();
  let numSquare = getNumSquares(selectLevel.value);
...
function resetGame() {
  ...
  score = 0;
  isGameActive = true; // Riabilita il gioco
  scoreCounter.innerHTML = score.toString().padStart(4, "0");
}

function getNumSquares(level) {
  switch (level) {
    case "easy":
      return 100;
    case "medium":
...
function drawSquare(squareIndex, numSquare, bombs) {
  ...
  square.addEventListener("click", function () {
    if (!isGameActive || this.classList.contains("clicked")) return;
    ...
    if (bombs.includes(squareIndex + 1)) {
      ...
      const bombsNearby = calculateBombsNearby(
        squareIndex,
        numSquare,
        squareWidth,
        bombs
      ...
      if (bombsNearby >= 0) {
        this.textContent = bombsNearby;
        this.classList.add(`number-${bombsNearby}`);
      }
      updateScore();
    }
  });
...
function gameOver(bombs) {
  ...
  isGameActive = false; // Disabilita ulteriori clic sulle caselle
  score = 0;
  scoreCounter.innerHTML = score.toString().padStart(4, "0");
}
...
function revealAllBombs(bombs) {
  bombs.forEach((bombIndex) => {
    const bombSquare = playGround.children[bombIndex - 1];
    bombSquare.classList.add("bg-danger", "clicked");
    bombSquare.innerHTML = '<i class="fa-solid fa-bomb"></i>';
  });
}
...
(function () {
  ...
  function glass() {
    ...
    async function panelRunner(color, time) {
      ...
      await delay(time);
      panelRunner(
        tinycolor.random().desaturate(10).darken(10).toHexString(),
        time
      );
    }
  }
  ...
  async function radiator() {
    ...
    async function radiate(points, time, color) {
      ...
      if (
        points.length > 1 &&
        points.filter(
          (p) => p.col < 0 || p.row < 0 || p.col > COLS || p.row > ROWS
        ...
        points.forEach((p, index, points) => {
          switch (p.type) {
            case "end-bottomRight":
              ...
              newPoints.push(
                new Point(p.row + 1, p.col + 1, "end-bottomRight")
              ); //end
              break;
            case "end-bottomLeft":
  ...
  function particles() {
    ...
    for (let i = 0; i < 15; i++) {
      ...
      moveDiagonally(
        p,
        time,
        seed > 0.5 ? theme.solid1 : theme.solid2,
        theme.base,
        direction
      );
    }
